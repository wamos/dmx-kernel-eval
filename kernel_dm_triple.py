...
class ner_reshape_casting(torch.nn.Module):
    def __init__(self):
        super(ner_reshape_casting, self).__init__()

    def forward(self, x):
        y = torch.mul(x,0.5)
        y = torch.reshape(y, (4096, 768))#128,768,16
        y = y.type(torch.CharTensor)        
        return y
...
def acc_kernel_emulation(state_name, state_shape, qid):
    ...
    print(f"acc kernel emulation, pid {os.getpid()}")    
    ...
    if state_name == "mel_scale":
        ...
        delayed_secs = 8.095*0.001
    elif state_name == "ner_reshape_casting":
        delayed_secs = 9.342*0.001
    else:
        delayed_secs = 0.010
    print(f"{state_name}: emulated acc kernel execution delay {delayed_secs}")
...
def kernel_emulation(state_name, state_shape, qid):
    q_name = f"{state_name}{qid}"
    proc_start = time.time()
    print(f"kernel emulation, pid {os.getpid()}")    
    state_q = InterProcessQueue('/'+q_name, state_shape, qid)
    mq = posix_ipc.MessageQueue('/'+q_name, 0)
    state_q.init_queue(mq)
    time_list = np.zeros(iterations)
...
def datamotion_fn(state_name, state_shape, qid, num_threads):
    ...
    if state_name == "mel_scale":
        ...
        model = ner_reshape_casting().to(device)
    else:
        model = mel_scale().to(device)
...
if benchmark_name == "mel_scale":
    ...
    shape = (4, 1024, 512)
    max_msg_size = 12582912
elif benchmark_name== "ner_reshape_casting":
    shape = (512,768,8)
    max_msg_size = 12582912
...
# TODO: we'll need two msg-queues per triple
...
dm_handle_list = []
kernel_handle_list = []
# TODO: change the launching order here
# each triple of kernels now have two queus, qid needs to be +2 every iteration
# kernel-1->DM-1-> kernel-2->DM-2->
#kernel_emulated == True
#acc_emulated = 
for qid in range(num_kernels):
    state_name = f"{benchmark_name}"

    if acc_emulated == False:
        print("kernel_emulation")
        kernel_handle = mp.Process(target=kernel_emulation, args=(state_name, shape, qid))
    else:
        print("acc_kernel_emulation")
