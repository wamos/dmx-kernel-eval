...
# def producer_emulation(state_name, state_shape, qid):
...
#     delay_secs = qid*0.010

#     loop_start = time.time()    
#     for i in range(iterations):    
#         start  = time.time()    
#         # qid is large you sleep longer
...
#     print(f"producer-emulation-{qid}, exec:{loop_end-loop_start}, overhead:{loop_start-proc_start}")

#     print(f"producer-emulation-{qid} joining now!") 
...
if exp_name != "mix-match":
    print("wrong script!")
    exit()

dmx_list = ["mel_scale", "reshape_casting", "image_resize", "concat_cast_flatten_aes", "concat_cast_flatten_gzip"]
if num_kernels == 5: 
    dmx_list = dmx_list * 1
elif num_kernels == 10:
    dmx_list = dmx_list * 2
...
for benchmark_name in dmx_list:
    ...
    max_msg_size_list.append(max_msg_size)   
    max_msg_count_list.append(max_msg_count)
...
for qid in range(num_kernels):
    benchmark_name = dmx_list[qid]
    max_msg_size = max_msg_size_list[qid]
    max_msg_count = max_msg_count_list[qid]
